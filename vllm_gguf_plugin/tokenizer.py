...
"""Restore the added tokens that transformers' GGUF converters drop.

A GGUF vocab tags every entry in ``tokenizer.ggml.token_type``. CONTROL (3)
and USER_DEFINED (4) are the two kinds HF keeps in ``added_tokens_decoder``:
strings matched verbatim, ahead of BPE, so they always encode to one id.
